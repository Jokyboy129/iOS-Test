...
class AudioEngineManager: ObservableObject {
    ...
    // Real-Time Synth & DSP Smoother State
    ...
    private func createWavData(from floats: [Float], sampleRate: Int32) -> Data {
        let numChannels: Int16 = 1
        let bitsPerSample: Int16 = 16
        let numSamples = Int32(floats.count)
        let subChunk2Size = numSamples * Int32(numChannels) * Int32(bitsPerSample / 8)
        let chunkSize = 36 + subChunk2Size

        var header = Data()
        header.append(contentsOf: [UInt8]("RIFF".utf8))
        header.append(withUnsafeBytes(of: chunkSize) { Data($0) })
        ...
        header.append(withUnsafeBytes(of: Int16(1)) { Data($0) })
        header.append(withUnsafeBytes(of: numChannels) { Data($0) })
        ...
        header.append(withUnsafeBytes(of: subChunk2Size) { Data($0) })

        var pcmData = Data(capacity: Int(subChunk2Size))
        for val in floats {
    ...
    private func generatePingPongWavData(buffer: [Float], sampleRate: Double, duration: Double) -> Data? {
        if buffer.isEmpty { return nil }
        let frameCount = Int(duration * sampleRate)
        var outBuffer = [Float](repeating: 0, count: frameCount)

        for i in 0..<frameCount {
            let idx = getPingPongIndex(index: i, count: buffer.count)
            var env: Float = 1.0
            let fadeFrames = Int(sampleRate * 0.15) // 150ms fade to prevent clicks
            if i < fadeFrames { env = Float(i) / Float(fadeFrames) }
            else if i > frameCount - fadeFrames { env = Float(frameCount - i) / Float(fadeFrames) }
            outBuffer[i] = buffer[idx] * env
        }
        return createWavData(from: outBuffer, sampleRate: Int32(sampleRate))
    }
    
    private func getPingPongIndex(index: Int, count: Int) -> Int {
        if count <= 0 { return 0 }
        let safeIndex = abs(index) // Prevents completely destructive crashes on negative overflow
    ...
    func addFile(url: URL, isAlarm: Bool = false) {
        ...
        } catch { print("File loading error: \(error)") }
    }
    ...
    func addAppleMusic(items: [MPMediaItem], isAlarm: Bool = false) {
        for item in items {
            ...
            } catch { print("Failed to load Apple Music track: \(error)") }
        }
    }
    ...
    func playBreathingCue(type: String, duration: Double = 0.0, isAnchor: Bool = false) {
        ...
        // Dynamically Generate Independent Ping-Pong Buffer if Real Breathing is Enabled
        if useRealBreathing && isBreathAction && duration > 0 {
            let buffer = type == "INHALE" ? realInhaleBuffer : realExhaleBuffer
            if let wavData = generatePingPongWavData(buffer: buffer, sampleRate: sampleRate, duration: duration) {
                do {
                    let player = try AVAudioPlayer(data: wavData)
                    player.volume = Float(masterVolume * dynamicVolumeMultiplier)
                    player.pan = getPanPos(mode: panBreathIndex, time: 0)
                    breathingPlayer?.stop()
                    player.play()
                    breathingPlayer = player
                } catch { print("WAV Gen Error: \(error)") }
                return
            }
        }

        // Fallback to static WAV playback
    ...
    func startBreathingExercise(inhale: Int, hold1: Int, exhale: Int, hold2: Int) {
        ...
        breathingTask = Task {
            while !Task.isCancelled {
                await MainActor.run { currentBreathingPhase = "Inhale (\(inhale)s)"; playBreathingCue(type: "INHALE", duration: Double(inhale)) }
                ...
                await MainActor.run { currentBreathingPhase = "Exhale (\(exhale)s)"; playBreathingCue(type: "EXHALE", duration: Double(exhale)) }
    ...
    private func setupAudio() {
        ...
        sourceNode = AVAudioSourceNode { [weak self] _, _, frameCount, audioBufferList -> OSStatus in
            ...
            if self.nNoise == 0 || self.nBeat == 0 { return noErr }
            
            let config = self.profiles[self.selectedProfileIndex]
            
            // Apply DSP Smoothers to avoid Zippering/Clicking when sliders move
            ...
            let targetsZero = targetVHeart == 0 && targetVClock == 0 && targetVBrown == 0 && targetVBreath == 0
            let smoothsZero = self.smoothedVHeart < 0.001 && self.smoothedVClock < 0.001 && self.smoothedVBrown < 0.001 && self.smoothedVBreath < 0.001

            let dt = 1.0 / self.sampleRate
            
            if targetsZero && smoothsZero {
                self.smoothedVHeart = 0
                self.smoothedVClock = 0
                self.smoothedVBrown = 0
                self.smoothedVBreath = 0
                ...
                let elapsedFrameTime = dt * Double(frameCount)
                ...
                if self.enableSlowdown {
                    self.currentDynamicBPM -= bpmDropRate * Double(frameCount)
                    if (bpmDropRate > 0 && self.currentDynamicBPM < self.targetBPM) || (bpmDropRate < 0 && self.currentDynamicBPM > self.targetBPM) {
                        self.currentDynamicBPM = self.targetBPM
                    }
                }
                let beatDuration = 60.0 / self.currentDynamicBPM
                self.tBeat = fmod(self.tBeat + elapsedFrameTime, beatDuration)
            ...
            for frame in 0..<Int(frameCount) {
                // Update smoothers every frame for click-free volume adjustment
                ...
                let vHeart = self.smoothedVHeart
                ...
                // Moved completely to Double Precision to protect fmod from 32-bit NaN crashing
                ...
                if self.enableSlowdown {
                    ...
                    let combinedLub = Float((lub + subLub) * 0.8)
                ...
                if vClock > 0 {
                    var clkWave: Float = 0
                    if self.syncClock {
                        if self.clkPlayIdx < self.clk.count { clkWave += self.clk[self.clkPlayIdx]; self.clkPlayIdx += 1 }
                        if self.clkPlayIdx2 < self.clk.count { clkWave += self.clk[self.clkPlayIdx2]; self.clkPlayIdx2 += 1 }
                    } else {
                        let idxClock = currentFrame % self.clk.count
                        clkWave = self.clk[idxClock]
                    }
                ...
                if vBreath > 0 {
                    ...
                    if self.syncBreathing {
                        ...
                        let breathDuration = 6.0
                        let breathPhase = fmod(timeInSeconds, breathDuration) / breathDuration
                        
                        if breathPhase < 0.45 {
                            inhale = Float(sin(Double.pi * (breathPhase / 0.45)))
                            sampleIdxForRealBreath = Int((breathPhase / 0.45) * (0.45 * breathDuration * self.sampleRate))
                        } else if breathPhase >= 0.5 && breathPhase < 0.95 {
                            exhale = Float(sin(Double.pi * ((breathPhase - 0.5) / 0.45)))
    ...
    private func rebuildPrototypes() {
        ...
        let nClockProto = Int(sampleRate * 1.5)
        ...
        Form {
            ...
            Section(header: Text("Procedural Layer Mixer").accessibilityHidden(true)) {
                ...
                VStack(alignment: .leading) {
                    ...
                    // You can adjust where the breathing is using this picker right below the volume!
        ...
        Form {
            ...
            Section(header: Text("Voice Preferences")) {
                ...
                Toggle("Use Real Breathing Sounds", isOn: $engine.useRealBreathing)
                    .accessibilityHint("Replaces voice cues with real breathing recordings. During 'Hold' phases, silence is played.")
            }
        }
    }
}
