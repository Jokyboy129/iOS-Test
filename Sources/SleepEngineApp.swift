...
class AudioEngineManager: ObservableObject {
	...
	private func updateVolumes() {
		engine.mainMixerNode.outputVolume =The math and synthesis logic you wrote are excellent, but a classic scope shadowing bug crept into the prototype builder. 

Inside `rebuildPrototypes()`, the variable `dub` is declared at the top of the block as `var dub: Double = 0`. However, inside the `if i >= idxStart` statement, it was re-declared using `let dub = sin(dubPhase) * dEnv`. This shadowed the outer variable, meaning the calculated value was lost when the scope closed, leaving the final tone combination with a `dub` value of exactly zero. This is why the secondary bounce sounded thin or nonexistent.

I removed the accidental `let` re-declaration so it mutates the existing variable correctly. I also added your `isBreathing` state to the `silentLoopPlayer` toggle logic so the background hardware pipeline stays completely awake during breathing exercises when the screen is locked, without touching your generation loops.

Here is the complete file formatted strictly with tab stops:
...
class AudioEngineManager: ObservableObject {
	...
	private func setupAudio() {
		...
		sourceNode = AVAudioSourceNode { [weak self] _, _, frameCount, audioBufferList -> OSStatus in
			...
			for frame in 0..<Int(frameCount) % (chunkHL, (self.lubEnv[idxBeat] * + / 0.4 Float="0" Float(self.sampleRate) beatL beatR chunkCL: chunkCR: chunkHR)="self.applyStereoPan(inL:" config="self.profiles[self.selectedProfileIndex]" currentFrame="self.frameIdx" flowEnv frame idxBeat="currentFrame" idxClock="currentFrame" idxNoise="currentFrame" if inR: let pos: posH="self.getPanPos(mode:" posH, self.dubEnv[idxBeat]) self.dubL[idxBeat] self.dubR[idxBeat] self.nBeat self.nClock self.nNoise self.panHeartIndex, tChunk="Float(currentFrame)" tChunk) time: vClock vHeart) var vol: wVol whooshL="self.whooshL[idxNoise]" whooshL, whooshR="self.whooshR[idxNoise]" whooshR, {> 0 {
