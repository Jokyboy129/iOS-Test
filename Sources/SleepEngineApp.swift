...
struct SoundscapeRecommendation {
	let name: String
	let rationale: String
	let profileIndex: Int
	let rainVolume: Double
	let organicHeartbeatVolume: Double
	let heartbeatVolume: Double
	let clockVolume: Double
	let clickVolume: Double
	let softClickVolume: Double
	let brownVolume: Double
	let whiteVolume: Double
	let breathVolume: Double
	let faceBrushVolume: Double
	let binauralVolume: Double
	let binauralTypeIndex: Int
	let clockTypeIndex: Int
	let clickPatternIndex: Int
	let syncClock: Bool
	let syncClick: Bool
	let enableRSA: Bool
	let enableHSPMode: Bool
	let enableIntimateMode: Bool
	let enableSlowdown: Bool
	let targetBPM: Double
	let slowdownMinutes: Double
...
func getRecommendation(personality: Int, preference: Int, sensitivity: Int, intimacy: Int, environment: Int) -> SoundscapeRecommendation {
	...
	var profileIndex = 0
	var rainVol = 0.0
	var orgHeartbeatVol = 0.0
	var heartbeatVol = 0.0
	var clockVol = 0.0
	var clickVol = 0.0
	var softClickVol = 0.0
	var brownVol = 0.0
	var whiteVol = 0.0
	var breathVol = 0.0
	var faceBrushVol = 0.0
	var binauralVol = 0.0
	var binauralType = 0
	var clockType = 0
	var clickPattern = 0
	...
	switch (personality, preference) {
	case (0, 0): // Anxious, Deep Sleep
		name = String(localized: "Womb Simulation Blanket")
		rationale = String(localized: "A heavy, low-frequency acoustic blanket designed to silence anxious racing thoughts and guide you into a secure sleep.")
		profileIndex = 2 // Womb Simulation (55 BPM)
		rainVol = 0.60
		orgHeartbeatVol = 0.50
		heartbeatVol = 0.80
		brownVol = 0.70
		binauralVol = 0.50
		binauralType = 0 // Delta
		hsp = true
		intimate = true
		slowdown = true
		tgtBPM = 35.0
		slowdownMin = 45.0
		haptics = true
		imports = [String(localized: "Heavy Waterfall"), String(localized: "Steady Airplane Cabin")]
		impReason = String(localized: "Adding a low rumble like a waterfall mask will drown out any sharp room noises and help stabilize sensory overload.")
		
	case (0, 1): // Anxious, ASMR & Intimacy
		name = String(localized: "Whispering Cradle")
		rationale = String(localized: "Designed to provide close, comforting vocal presence and soft ASMR friction to ease loneliness and anxiety.")
		profileIndex = 0 // ASMR Blood Flow (60 BPM)
		heartbeatVol = 0.60
		faceBrushVol = 0.85
		breathVol = 0.70
		softClickVol = 0.40
	...
	case (0, 2): // Anxious, Calm & Grounding
		name = String(localized: "Somatic Breath Anchor")
		rationale = String(localized: "Uses clear, steady rhythmic breathing and warm heart pulses to bring your focus back to your body.")
		profileIndex = 3 // Zen Meditation (50 BPM)
		heartbeatVol = 0.75
		breathVol = 0.85
		rainVol = 0.40
		syncBreath = true
		rsa = true
		imports = [String(localized: "Tibetan Singing Bowl"), String(localized: "Rain on Tent")]
		impReason = String(localized: "A singing bowl provides a resonant, long-decay frequency that acts as a perfect focal point for mindful grounding.")

	case (0, 3): // Anxious, Focus & Study
		name = String(localized: "Concentration Dome")
		rationale = String(localized: "Alpha brainwave frequencies and a regular rhythmic tick block anxious chatter, creating an optimal study space.")
		profileIndex = 4 // Athletic Recovery (45 BPM)
		clockVol = 0.35
		clockType = 1 // Pocket Watch
		brownVol = 0.45
		binauralVol = 0.60
		binauralType = 2 // Alpha
		syncClk = true
		imports = [String(localized: "Library Rain"), String(localized: "Distant Train Tracks")]
		impReason = String(localized: "The subtle clatter of a distant train provides an engaging, loop-like pattern that keeps the brain's background chatter occupied.")

	case (0, 4): // Anxious, Stress Relief
		name = String(localized: "Vaporous Rain Relief")
		rationale = String(localized: "An acoustic refuge with heavy softening to melt away somatic tension and lower nervous system arousal.")
		profileIndex = 5 // Gentle Drift (42 BPM)
		rainVol = 0.80
		orgHeartbeatVol = 0.60
		breathVol = 0.70
		hsp = true
		rsa = true
		imports = [String(localized: "Campfire Crackle"), String(localized: "Purring Cat")]
		impReason = String(localized: "Campfire crackles introduce organic high-frequency clicks that signal safety and cozyness to the anxious brain.")

	case (1, 0): // Stressed, Deep Sleep
		name = String(localized: "Overloaded Mind Decompression")
		rationale = String(localized: "Strong binaural entrainment and thick brown noise to wash away the cortisol and demands of a high-pressure day.")
		profileIndex = 7 // Hibernation State (35 BPM)
		brownVol = 0.85
		heartbeatVol = 0.70
		binauralVol = 0.70
		binauralType = 0 // Delta
		...
		slowdownMin = 60.0
		hsp = true
		imports = [String(localized: "Whirring Fan"), String(localized: "Soft Brown Noise Drone")]
		impReason = String(localized: "A steady mechanical fan sound provides a dense barrier of white-brown noise to prevent any stressful alerts from waking you.")

	case (1, 1): // Stressed, ASMR & Intimacy
		name = String(localized: "Solfeggio Soft Touch")
		rationale = String(localized: "Slow, tactile face brushing and synchronized taptic heartbeats to ground you in deep bodily sensory pleasure.")
		profileIndex = 11 // Soft Pillowy Pulse (62 BPM)
		faceBrushVol = 0.90
		heartbeatVol = 0.50
		softClickVol = 0.45
		intimate = true
		haptics = true
		imports = [String(localized: "Aura Cleansing Drone"), String(localized: "Gentle Wind Chimes")]
		impReason = String(localized: "A rich synthesizer pad or drone introduces a harmonic layer that acts as a therapeutic sound bath.")

	case (1, 2): // Stressed, Calm & Grounding
		name = String(localized: "Quiet Forest Sanctuary")
		rationale = String(localized: "Slowing down your biological clock with a grandfather ticking and breathing cues matched to a relaxed resting state.")
		profileIndex = 1 // Standard Resting Heart (72 BPM)
		clockVol = 0.40
		clockType = 2 // Grandfather Clock
		breathVol = 0.75
		syncClk = true
		rsa = true
		imports = [String(localized: "Forest Birds"), String(localized: "Gentle Stream")]
		impReason = String(localized: "The sounds of a forest stream encourage natural breathing pacing and lower stress levels immediately.")

	case (1, 3): // Stressed, Focus & Study
		name = String(localized: "Flow State Generator")
		rationale = String(localized: "A focus-enhancing blend of theta brainwaves and rhythmic clicks to induce high productivity without stress.")
		profileIndex = 3 // Zen Meditation (50 BPM)
		clickVol = 0.35
		binauralVol = 0.55
		binauralType = 1 // Theta
		syncClkTrk = true
		imports = [String(localized: "Lo-Fi Beats"), String(localized: "Cafe Ambiance")]
		impReason = String(localized: "Lo-fi instrumental beats overlay perfectly with the generator to keep you in an uninterrupted flow state.")

	case (1, 4): // Stressed, Stress Relief
		name = String(localized: "Cortisol Meltdown")
		rationale = String(localized: "Combining slow-wave heartbeat, heavy rain, and vocal anchors to reset your nervous system from fight-or-flight.")
		profileIndex = 9 // Slow Wave Sleep (25 BPM)
		rainVol = 0.85
		breathVol = 0.80
		heartbeatVol = 0.60
		hsp = true
		anchors = true
		imports = [String(localized: "Gentle Ocean Waves"), String(localized: "Tibetan Singing Bowls")]
		impReason = String(localized: "Ocean waves follow a natural breathing rhythm that will coax your lungs and mind into an organic state of relaxation.")

	case (2, 0): // Tired, Deep Sleep
		name = String(localized: "Deep Sleep Dive")
		rationale = String(localized: "For the utterly exhausted. Absolute room masking white noise combined with slow-wave sleep pulses for immediate blackout.")
		profileIndex = 9 // Slow Wave Sleep (25 BPM)
		whiteVol = 0.80
		brownVol = 0.50
		heartbeatVol = 0.80
		...
		binauralType = 0 // Delta
		hsp = true
		imports = [String(localized: "Heavy Air Conditioner"), String(localized: "Pink Noise")]
		impReason = String(localized: "Heavy AC unit rumble acts as a powerful audio shield, ensuring you stay in deep, uninterrupted slow-wave sleep.")

	case (2, 1): // Tired, ASMR & Intimacy
		name = String(localized: "Somnolent Whispers")
		rationale = String(localized: "A very soft, slow heartbeat with whispering vocal guides to lull you to sleep when you are too tired to think.")
		profileIndex = 6 // Deep Sleep Resonance (40 BPM)
		heartbeatVol = 0.50
		breathVol = 0.80
		faceBrushVol = 0.60
		intimate = true
	...
	case (2, 2): // Tired, Calm & Grounding
		name = String(localized: "Tired Heart Rest")
		rationale = String(localized: "A resting rhythm with a pocket watch tick to ground your weary mind and provide comfort as you drift away.")
		profileIndex = 5 // Gentle Drift (42 BPM)
		clockVol = 0.35
		clockType = 1 // Pocket Watch
		heartbeatVol = 0.70
		syncClk = true
		imports = [String(localized: "Purring Cat"), String(localized: "Distant Wind Chimes")]
		impReason = String(localized: "A purring cat is proven to lower heart rate and blood pressure, acting as an exceptional acoustic pacifier.")

	case (2, 3): // Tired, Focus & Study
		name = String(localized: "Recharge Station")
		rationale = String(localized: "When you are tired but must stay awake to finish a task. Beta-focused alpha waves to keep your tired mind alert.")
		profileIndex = 1 // Standard Resting Heart (72 BPM)
		whiteVol = 0.40
		binauralVol = 0.70
		binauralType = 2 // Alpha
		imports = [String(localized: "Upbeat Lo-Fi"), String(localized: "Coffee Shop White Noise")]
		impReason = String(localized: "Bustling coffee shop noise provides high-frequency chatter that stimulates attention even when your body is fatigued.")

	case (2, 4): // Tired, Stress Relief
		name = String(localized: "Exhausted Recovery")
		rationale = String(localized: "Restores reserves with heavy rain and a womb-like low heartbeat to repair a drained system.")
		profileIndex = 2 // Womb Simulation (55 BPM)
		rainVol = 0.75
		orgHeartbeatVol = 0.50
		heartbeatVol = 0.60
		hsp = true
		imports = [String(localized: "Summer Forest Rain"), String(localized: "Cosmic Drone")]
		impReason = String(localized: "A synthesized cosmic drone introduces low-frequency warmth that feels like a gentle vibrational massage for the mind.")

	case (3, 0): // Calm, Deep Sleep
		name = String(localized: "Ethereal Sleep Resonance")
		rationale = String(localized: "For the naturally calm. A minimalist, low-BPM pulse with delta brainwave induction for deep spiritual sleep.")
		profileIndex = 10 // Cinematic Oceanic (18 BPM)
		heartbeatVol = 0.80
		binauralVol = 0.45
		binauralType = 0 // Delta
		imports = [String(localized: "Deep Ocean Drone"), String(localized: "Submarine Sonar Ping")]
		impReason = String(localized: "An oceanic drone complements the ultra-slow cinematic pulse, expanding the spatial feel of your sleep sanctuary.")

	case (3, 1): // Calm, ASMR & Intimacy
		name = String(localized: "Gentle ASMR Drift")
		rationale = String(localized: "Very light, spacious ASMR brushing and breathing for a tranquil, floating bedtime experience.")
		profileIndex = 5 // Gentle Drift (42 BPM)
		faceBrushVol = 0.75
		breathVol = 0.50
		intimate = true
		imports = [String(localized: "Wind in Trees"), String(localized: "Dry Leaves Rustling")]
		impReason = String(localized: "Ruffling dry leaves or wind rustling adds an organic high-frequency texture that heightens tactile ASMR sensitivity.")

	case (3, 2): // Calm, Calm & Grounding
		name = String(localized: "Zen Garden Oasis")
		rationale = String(localized: "The perfect companion for mindfulness. A rhythmic zen pulse and slow breathing synchronized to your heartbeat.")
		profileIndex = 3 // Zen Meditation (50 BPM)
		breathVol = 0.70
		syncBreath = true
		rsa = true
		imports = [String(localized: "Dripping Water Fountain"), String(localized: "Tibetan Bell")]
		impReason = String(localized: "A dripping fountain provides a naturally random, soothing click pattern that represents the flow of life.")

	case (3, 3): // Calm, Focus & Study
		name = String(localized: "Deep Focus Sanctuary")
		rationale = String(localized: "A crisp pocket watch and high-concentration alpha waves for deep creative flow and mental clarity.")
		profileIndex = 3 // Zen Meditation (50 BPM)
		clockVol = 0.30
		clockType = 1 // Pocket Watch
		binauralVol = 0.50
		binauralType = 2 // Alpha
		syncClk = true
		imports = [String(localized: "Focus Piano Chords"), String(localized: "Gentle Rain on Skylight")]
		impReason = String(localized: "Gentle piano chords or ambient pads help stimulate creative thinking and keep your focus locked in.")

	case (3, 4): // Calm, Stress Relief
		name = String(localized: "Tranquil Rain Sanctuary")
		rationale = String(localized: "Keeps your calm state protected with a gentle rain layer and soft somatic chest-wall heartbeats.")
		profileIndex = 6 // Deep Sleep Resonance (40 BPM)
		rainVol = 0.60
		orgHeartbeatVol = 0.40
		breathVol = 0.50
		imports = [String(localized: "Wind Chimes in Breeze"), String(localized: "Warm Crackling Hearth")]
		impReason = String(localized: "Wind chimes add melodic, non-periodic notes that capture the wandering mind and dissolve lingering stress.")

	case (4, 0): // Restless, Deep Sleep
		name = String(localized: "Hyperactive Mind Grounding")
		rationale = String(localized: "Using complex rhythmic structures (synchronized clicks and ticking) to lock in the restless mind, combined with delta waves to force sleep.")
		profileIndex = 2 // Womb Simulation (55 BPM)
		clockVol = 0.40
		clockType = 0 // Quartz Wall Clock
		clickVol = 0.50
		softClickVol = 0.40
		brownVol = 0.60
		binauralVol = 0.60
		...
		syncClk = true
		syncClkTrk = true
		clickPattern = 1 // Tick-Tock
		imports = [String(localized: "Static Fan Hum"), String(localized: "Rhythmic Trains")]
		impReason = String(localized: "A mechanical fan or trains offer a complex yet predictable acoustic framework that anchors a restless mind and prevents distraction.")

	case (4, 1): // Restless, ASMR & Intimacy
		name = String(localized: "ASMR Tactile Focus")
		rationale = String(localized: "High-frequency click tracks and intense face brushing that give the sensory-seeking brain rich, nearby stimulation.")
		profileIndex = 0 // ASMR Blood Flow (60 BPM)
		faceBrushVol = 0.95
		softClickVol = 0.60
		clickVol = 0.40
		clickPattern = 2 // Tock-Tick
		intimate = true
		imports = [String(localized: "Scratching Textures"), String(localized: "Tapping on Wood")]
		impReason = String(localized: "Wood tapping or scratching introduces fast transients that satisfy the sensory-seeking mind, facilitating relaxation.")

	case (4, 2): // Restless, Calm & Grounding
		name = String(localized: "ADHD Somatic Pacer")
		rationale = String(localized: "Using highly-defined heartbeat positions and structured clock ticks to physically pace and calm a hyperactive body.")
		profileIndex = 11 // Soft Pillowy Pulse (62 BPM)
		clockVol = 0.45
		clockType = 3 // Metronome
		heartbeatVol = 0.80
		syncClk = true
		haptics = true
		imports = [String(localized: "Steady Pendulum"), String(localized: "Ocean Shoreline")]
		impReason = String(localized: "A steady pendulum sound reinforces the Metronome, creating a powerful, hypnotizing physical rhythm that guides the body to rest.")

	case (4, 3): // Restless, Focus & Study
		name = String(localized: "Hyperfocus Engine")
		rationale = String(localized: "Blocks out all external distractions with powerful white noise masking and high-frequency theta waves to trigger hyperfocus.")
		profileIndex = 1 // Standard Resting Heart (72 BPM)
		whiteVol = 0.70
		binauralVol = 0.65
		binauralType = 1 // Theta
		clickVol = 0.30
		syncClkTrk = true
		imports = [String(localized: "Brownian Noise"), String(localized: "Office Keyboard Chatter")]
		impReason = String(localized: "Keyboard chatter or heavy brownian noise acts as a continuous wall of sound, masking sudden visual or audio distractions.")

	case (4, 4): // Restless, Stress Relief
		name = String(localized: "Rhythmic Rain Escape")
		rationale = String(localized: "Captures a busy mind using a thick layer of organic rain combined with structured breathing holds to disrupt stress feedback loops.")
		profileIndex = 4 // Athletic Recovery (45 BPM)
		rainVol = 0.80
		breathVol = 0.75
		clockVol = 0.30
		...
		syncClk = true
		anchors = true
		imports = [String(localized: "Rain on Tin Roof"), String(localized: "Binaural Rain Beats")]
		impReason = String(localized: "Rain on tin roof has complex, rich high-frequency dynamics that soothe the ADHD brain with pleasant white-like noise.")

	default:
		name = String(localized: "Standard Calm")
		rationale = String(localized: "A balanced resting soundscape to soothe your nervous system.")
		profileIndex = 1
	}
